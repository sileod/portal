...
New hub setup also asks for a short hub name, defaulting to `portal`. That name is used for the Funnel/MagicDNS hostname and the hub's Portal host label, so the normal public URL is `https://portal.<tailnet>.ts.net` when available.
...
- vertical tab-bar width, defaulting to 220px
- tmux status-bar background color, applied to Portal-managed sessions across connected hosts

The browser UI and login page use the 🌀 Portal favicon.
...
Control operations are acknowledged by the target terminal host. If a hub has been updated but a terminal host has not, the UI reports which host needs an update rather than leaving an operation stuck in an optimistic state.

## Naming

Portal names are independent of the operating-system hostname.

Change only the label used by Portal tabs:

```bash
portal host shortname
```

On a Tailscale Funnel hub, change both the Portal label and the Funnel/MagicDNS machine hostname:

```bash
portal host shortname --tailscale
```

This does not change the machine's OS hostname. The Portal daemon reconnects under the new label and the saved central URL is updated after Tailscale publishes the new name.

For explicit hub creation, the Funnel hostname and Portal host label can also be supplied separately:

```bash
PORTAL_PASSWORD='your password' \
PORTAL_HOST='hub-label' \
PORTAL_FUNNEL_HOST='public-name' \
...
```bash
portal link https://your-portal-url --password 'your password' --host shortname
```

Or simply run `portal` and enter the URL, password, and optional host label interactively.
